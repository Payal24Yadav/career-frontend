...
export default function EditBlogPage() {
  ...
  const [form, setForm] = useState({
    ...
    status: "published", isFeatured: false, publishDate: "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await getBlogsAPI(`limit=100&admin=true`);
        setAllBlogs(data.data);
        const blog = data.data.find((b: any) => b._id === params.id);
        if (blog) {
          // Backward compatibility check: if old blog string, convert to single text block or leave as array
          let parsedContent = blog.content;
          if (typeof blog.content === 'string') {
            parsedContent = [{ type: 'Heading/Paragraph', sectionHeading: "", sectionParagraph: blog.content }];
          } else if (!Array.isArray(blog.content)) {
            parsedContent = [];
          }

          setForm({
            title: blog.title, slug: blog.slug, description: blog.description, content: parsedContent, category: blog.category,
            author: blog.author,
            ...
            relatedBlogs: blog.relatedBlogs ? blog.relatedBlogs.map((r: any) => typeof r === "string" ? r : r._id) : []
          });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateBlogAPI(params.id as string, payload);
      router.push("/admin/blogs");
    } catch (err: any) { alert(err.response?.data?.message || "Failed to update blog"); } finally { setLoading(false); }
  };
    ...
    <div className="pb-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <button type="submit" disabled={loading} className="flex items-center gap-2 px-8 py-4 bg-primary text-white font-bold rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 shadow-lg shadow-primary/25"><Save className="w-5 h-5" /> {loading ? "Updating..." : "Update Blog"}</button>
        </div>
      </form>
    </div>
  );
